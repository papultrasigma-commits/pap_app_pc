...
import { Play, Heart, MessageCircle, Share2, MoreVertical, Image as ImageIcon, Video, Send, CheckCircle2, Loader2, X, Trash2, ShieldAlert, Shield, Clock, Flag } from 'lucide-react';
...
const PostCard = ({ post, currentUser, currentUserName, currentUserAvatar, isAdmin, onDelete }) => {
  ...
  const handleSendComment = async (e) => {
    ...
    try {
      ...
      if (!error && data) {
        const newComm = {
          ...data[0],
          profiles: { avatar_url: currentUserAvatar } 
        };
  ...
  // --- NOVA FUNÇÃO PARA ABRIR O MODAL DE REPORT (SEM WINDOW.PROMPT) ---
  const openReportModal = (type, id, authorName) => {
    setReportTarget({ type, id, name: authorName });
    setReportReason("");
        ...
        <div className="relative">
          ...
          {showPostMenu && (
            <div className="absolute right-0 mt-2 w-40 bg-[#1f2122] border border-gray-700 rounded-lg shadow-xl z-20 py-1 overflow-hidden">
              ...
              {!isAuthor && (
                <button 
                  onClick={() => { setShowPostMenu(false); openReportModal('post', post.id, post.author_name); }} 
          ...
          {post.media_type === 'video' ? (
            <video 
              ...
              onVolumeChange={(e) => {
                ...
                // Dispara o evento para todos os outros vídeos se atualizarem
                          ...
                          <div className="relative">
                            ...
                            {activeCommentMenu === c.id && (
                              <div className="absolute right-0 mt-1 w-32 bg-[#1f2122] border border-gray-700 rounded-lg shadow-xl z-30 py-1 overflow-hidden">
                                ...
                                {!isCommentAuthor && (
                                  <button 
                                    onClick={() => { setActiveCommentMenu(null); openReportModal('comment', c.id, c.author_name); }} 
          ...
          <div className="bg-[#181a1b] border border-gray-800 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleReportSubmit} className="p-5">
              <p className="text-sm text-gray-400 mb-4">
                Por que motivo estás a denunciar <strong className="text-white">{reportTarget.name}</strong>?
              </p>
              ...
              <div className="space-y-3 mb-6">
                {[
                  ...
                  { id: 'conteudo_inadequado', label: 'Conteúdo Inadequado (+18)' }
                ].map(motivo => (
...
export default function FeedPage() {
  ...
  const previewVideoRef = useRef(null); // Ref para o vídeo de preview

  // Sincronização do volume entre vídeos para a pré-visualização
  useEffect(() => {
    const syncVolume = (e) => {
              ...
              {selectedFile?.type === 'video' ? (
                <video 
                  ...
                  onVolumeChange={(e) => {
                    ...
                    // Dispara o evento para atualizar todos os outros vídeos no feed
                    window.dispatchEvent(new CustomEvent('sync-video-volume', {
                      detail: { volume: e.target.volume, muted: e.target.muted, source: e.target }
                    }));
                  }}
